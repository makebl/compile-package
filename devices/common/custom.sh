...
for ipk in $(find feeds/makebl/* -maxdepth 0 -type d);
do
...
./scripts/feeds install -a -p makebl/packages -f
./scripts/feeds install -a

rm -rf feeds/packages/lang/golang
svn export https://github.com/coolsnowwolf/packages/trunk/lang/golang feeds/packages/lang/golang

sed -i 's/\(page\|e\)\?.acl_depends.*\?}//' `find package/feeds/makebl/packages/luci-*/luasrc/controller/* -name "*.lua"`
# sed -i 's/\/cgi-bin\/\(luci\|cgi-\)/\/\1/g' `find package/feeds/makebl/packages/luci-*/ -name "*.lua" -or -name "*.htm*" -or -name "*.js"` &
...
sed -i \
	...
	package/feeds/makebl/packages/*/Makefile
